...
:::note
The listener is registered **before** `execute()` is called and buffers any results that arrive while the mutation is in-flight. Once the mutation response is received, the buffer is flushed against the predicate — then evaluation continues in real-time for every subsequent result. The predicate may therefore be **called multiple times** until it returns `true`; keep it a pure expression with no side effects. Use `waitOnTimeout` as a safety valve in case the expected event never arrives.
:::
