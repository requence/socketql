...
Wraps URQL's `Provider` and manages the WebSocket connection lifecycle. Calls `connect()` on mount and throws connection errors to the nearest React error boundary.
...
When the server rejects the connection, the provider throws the `ConnectionError` during render. Wrap it in an error boundary to handle rejections:
...
The same client can be reused after the error boundary resets — the provider will re-register its error listener and call `connect()` again.
