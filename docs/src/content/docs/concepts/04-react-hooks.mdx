...
:::note
The predicate is called once per incoming result — first for any events buffered while the mutation was in-flight, then in real-time — until it returns `true`. Keep it a pure, side-effect-free expression.
:::
