...
For cases where you need to match a specific event by data — such as waiting for a subscription event whose `id` matches the one returned by the mutation — pass a **predicate function** instead:

```tsx
function DuplicateItem({ knownId }: { knownId: string }) {
  const [, execute] = useMutation({
    query: DuplicateItemDocument,
    waitOn: (result, name) =>
  ...
  return <button onClick={() => execute({ id: knownId })}>Duplicate</button>
}
```
